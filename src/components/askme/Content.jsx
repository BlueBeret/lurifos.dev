            ...
            case 1:
                ...
                if (activeQuestion === id) {
                    ...
                    if (activeQuestion !== null) {
                        const elem = diary.current
                        const target = elem.querySelector(`#body-${activeQuestion}`)
                        const arrow = elem.querySelector(`#arrow-${activeQuestion}`)
                        arrow.classList.toggle("rotate-180")
                        target.classList.toggle('max-h-96')
                        await sleep(200);
    ...
    return (
        ...
        ${styles.customscroll}`}
            ...
            >
                ...
                {posts.map((x, i) => (
                    <div key={i} className={`transition mt-3 animate-fade-in-up duration-500 ${activeQuestion === x.uuid || activeQuestion === null ? '' : 'opacity-20'} ${activeQuestion === x.uuid ? 'scale-110 z-50' : 'scale-100'}`}>
                        ...
                        " onClick={(e) => toogleBody(e, x.uuid)}>
                            <h3 className="font-bold noselect text-sm sm:text-base">{x.question}</h3>
                            <div className="flex flex-row justify-start items-center gap-1 text-xs md:text-base mt-0">
                                <span className="text-gray-600">@{x.username} ·</span>
                        ...
                        <div id={`body-${x.uuid}`} className="transition-max-height duration-500 text-gray-600 slide overflow-hidden sm:overflow-clip text-sm sm:text-base">
                            ...
                            <ReactMarkdown remarkPlugins={[gfm]} className={styles.body}>
                                {truncate(x.answer, 256)}
                            </ReactMarkdown>
                            {x.answer.length > 256 ? <ReadMore x={x} /> : ''}
...
const ReadMore = ({ x }) => {
    ...
    return <a href={'/askme/' + x.uuid} className="text-sm text-blue-500" target='_blank' rel="noreferrer"> Read More </a>
}
